...
#            print (read.profile.end-read.profile.start)
    ...
    def single_sim(self):
        ...
        e_gun_energy = 3000 # measured in volts
        avg_velo = sqrt( (2.0 * e_gun_energy * e_charge) / e_mass) # m/s
        e_gun_angle = .1 # in degrees                

          
        positions = np.array([[0.000 ,0.001, -0.03, 0.0,]])
  #      velocities = np.array([[0.0 * avg_velo * sin(radians(e_gun_angle)), -1 * avg_velo * sin(radians(e_gun_angle)) ,  avg_velo * cos(radians(e_gun_angle)), 0.0]])        
...
#420^2 + (5e5)^2
        ...
        self.SINGLE_SIM.calculator = self._GPU(path_to_integrator, device_id = 0)
        ...
        x_conf = len(np.where( abs(self.SINGLE_SIM.r_vals[0][:,0]) < coil_1.radius)[0]) * dt * iter_nth * 1e9
        y_conf = len(np.where( abs(self.SINGLE_SIM.r_vals[0][:,1]) < coil_1.radius)[0]) * dt * iter_nth * 1e9
        z_conf = len(np.where( abs((coil_2.z_pos/2.0) - self.SINGLE_SIM.r_vals[0][:,2]) < (coil_2.z_pos/2.0))[0]) * dt * iter_nth * 1e9
        
        #print "X confinement time: ", x_conf, " ns"
        #print "Y confinement time: ", y_conf, " ns"
        #print "Z confinement time: ", z_conf, " ns"
        print "Total confinement:", np.amin([x_conf,y_conf,z_conf]), " ns"
        plt.title(("Total confinement:", int(np.amin([x_conf,y_conf,z_conf])), " ns"))
        plt.show()
...
#        velo_p_slice[:,0] = avg_velo * np.sin(thetas) # Set X components
        ...
        self.DIM_SIM.calculator = self._GPU('//Users/jonkelley/Desktop/Research in Physics/python/gpu_accelerate/potential_optimizer/part1.cl', device_id = 2)
