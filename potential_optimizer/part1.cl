...
inline double4 get_accel(double4 pos, double4 v, float4 local_coils, __global float* ee,__global float* ek) //where factor equals charge/mass
{	
	...
	for(int iter = 0; iter<(num_steps); iter++){
		...
		for (int sub_int = 0; sub_int < iter_nth; sub_int++){
			...
			*/
			...
			accel = get_accel(pos, velo, local_coils, ee_tab, ek_tab); //returns acceleration
			velo += (accel * dt);
			pos += (velo * dt);
